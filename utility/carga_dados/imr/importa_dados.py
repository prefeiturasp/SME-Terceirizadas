...
def importa_tipos_penalidade(
    ...
    try:
        processador = ProcessadorDePlanilhaIMR(
...
class TipoPenalidadeHelper:
    @staticmethod
    def get_edital(numero_edital: str):
        return Edital.objects.get(numero=numero_edital)

    @staticmethod
    def get_tipo_gravidade(tipo_gravidade: str):
        return TipoGravidade.objects.get(tipo=tipo_gravidade)
    ...
    def cria_tipo_penalidade_obj(
        ...
        edital = TipoPenalidadeHelper.get_edital(usuario_schema.edital)
        gravidade = TipoPenalidadeHelper.get_tipo_gravidade(usuario_schema.gravidade)
        status = usuario_schema.status == StatusAtivoInativo.ATIVO
        tipo_penalidade, _ = TipoPenalidade.objects.update_or_create(
            edital=edital,
            numero_clausula=usuario_schema.numero_clausula,
    ...
    @staticmethod
    def normaliza_obrigacoes(obrigacoes: str):
        if obrigacoes[-1] == ";":
            obrigacoes = obrigacoes[: len(obrigacoes) - 1]
        return obrigacoes.split(";")

    @staticmethod
    def cria_obrigacao_penalidade(tipo_penalidade: TipoPenalidade, descricao: str):
        ObrigacaoPenalidade.objects.create(
            tipo_penalidade=tipo_penalidade, descricao=descricao
        )

    @staticmethod
    def cria_tipo_penalidade(usuario_schema: ImportacaoPlanilhaTipoPenalidadeSchema):
        tipo_penalidade = TipoPenalidadeHelper.cria_tipo_penalidade_obj(usuario_schema)
        lista_obrigacoes = TipoPenalidadeHelper.normaliza_obrigacoes(
            usuario_schema.obrigacoes
        )
        for descricao_obrigacao in lista_obrigacoes:
            TipoPenalidadeHelper.cria_obrigacao_penalidade(
                tipo_penalidade, descricao_obrigacao
            )


class ProcessadorDePlanilhaIMR:
    LINHA_2 = 2
    ...
    def processamento(self):
        ...
        for ind, linha in enumerate(
            ...
            try:
                ...
                if isinstance(self.arquivo, ImportacaoPlanilhaTipoPenalidade):
                    TipoPenalidadeHelper.cria_tipo_penalidade(ind, usuario_schema)
            except Exception as exc:
                self.erros.append(f"Linha {ind} - {exc}")
