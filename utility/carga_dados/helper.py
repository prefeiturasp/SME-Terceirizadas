...
def excel_to_list(arquivo, in_memory=False):
    ...
    if in_memory:
        workbook = xlrd.open_workbook(
            filename=arquivo, file_contents=arquivo.read()
        )  # noqa
    else:
...
def adiciona_m2m_items(
    campo_m2m, dicionario_m2m, modelo, dicionario, campo_unico
):  # noqa
    """
    ...
    for id in dicionario_m2m:
        obj = get_modelo(
            modelo=modelo, modelo_id=id, dicionario=dicionario, campo_unico=campo_unico
        )
        campo_m2m.add(obj)
