...
def test_url_endpoint_cria_dias_suspensao_atividades(client_autenticado_coordenador_codae):
    data = {
        'data': '2022-08-08',
    ...
    response = client_autenticado_coordenador_codae.get(
        '/dias-suspensao-atividades/?mes=8&ano=2022',
        content_type='application/json'
    )
    assert response.status_code == status.HTTP_200_OK
    assert len(response.json()['results']) == 3
    response = client_autenticado_coordenador_codae.get(
        '/dias-suspensao-atividades/?mes=9&ano=2022',
        content_type='application/json'
    )
    assert response.status_code == status.HTTP_200_OK
