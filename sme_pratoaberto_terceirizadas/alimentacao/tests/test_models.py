...
def test_model_str_cardapio_inverter_dia_cardapio(cardapio, inverter_dia_cardapio):
    ...
    assert inverter_dia_cardapio.__str__() == str(inverter_dia_cardapio.uuid)
...
def test_validacao_final_semana(request_solicitar_errado, request_solicitar_certo):
    assert not InverterDiaCardapio.valida_fim_semana(request_solicitar_errado)
    assert InverterDiaCardapio.valida_fim_semana(request_solicitar_certo)
...
def test_validacao_se_ja_existe_cadastro(request_post):
    assert not InverterDiaCardapio.ja_existe(request_post)
