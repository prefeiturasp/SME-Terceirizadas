...
from sme_sigpae_api.dieta_especial.models import ArquivoCargaUsuariosEscola
...
class ProcessadorPlanilha:
    ...
    def monta_dicionario_de_dados(self, linha: tuple) -> dict:
        return {
            ...
            for index, key in enumerate(
                ArquivoCargaUsuariosDiretorSchema.schema()["properties"].keys()
            )
        }

    def cria_usuario(
        self, ind, usuario_schema: ArquivoCargaUsuariosDiretorSchema
    ):  # noqa C901
        try:
    ...
    @transaction.atomic
    def __criar_usuario_diretor(
        self, usuario_schema: ArquivoCargaUsuariosDiretorSchema
    ):
        escola = self.consulta_escola(usuario_schema.codigo_eol_escola)
        self.checa_usuario(
            usuario_schema.rf_diretor,
            usuario_schema.email_diretor,
            usuario_schema.nome_diretor,
        )
        self.__criar_usuario(
            escola, usuario_schema.nome_diretor, usuario_schema.email_diretor, "DIRETOR"
        )

    @transaction.atomic
    def __criar_usuario_assitente(
        self, usuario_schema: ArquivoCargaUsuariosDiretorSchema
    ):
        escola = self.consulta_escola(usuario_schema.codigo_eol_escola)
        self.checa_usuario(
            usuario_schema.rf_assistente,
            usuario_schema.email_assistente,
    ...
    def checa_usuario(self, registro_funcional, email, nome_planilha):
        ...
        if usuario:
            raise Exception(
                f"Usuário pra esse email {email} ou com esse rf "
...
def importa_usuarios_escola(
    usuario: Usuario, arquivo: ArquivoCargaUsuariosEscola
) -> None:
