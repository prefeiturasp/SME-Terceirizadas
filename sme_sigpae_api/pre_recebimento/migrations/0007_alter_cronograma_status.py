...
def atualiza_status(apps, _):
    ...
    Cronograma.objects.filter(status="ASSINADO_CRONOGRAMA").update(
        status="ASSINADO_E_ENVIADO_AO_FORNECEDOR"
    )
    Cronograma.objects.filter(status="ENVIADO_AO_FORNECEDOR").update(
        status="ASSINADO_E_ENVIADO_AO_FORNECEDOR"
    )
...
def backwards(apps, _):
    Cronograma = apps.get_model("pre_recebimento", "Cronograma")
    Cronograma.objects.filter(status="ASSINADO_E_ENVIADO_AO_FORNECEDOR").update(
