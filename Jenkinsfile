        ...
        stage('Deploy'){
            ...
            steps {
			    ...
			    sh 'kubectl rollout restart deployment/sigpae-frontend -n sme-sigpae-treino'
                    ...
                    withCredentials([file(credentialsId: "${kubeconfig}", variable: 'config')]){
                            sh('cp $config '+"$home"+'/.kube/config')
                            sh "kubectl rollout restart deployment/sigpae-backend -n ${namespace}"
                            sh "kubectl rollout restart deployment/sigpae-beat -n ${namespace}"
                            sh "kubectl rollout restart deployment/sigpae-celery -n ${namespace}"
